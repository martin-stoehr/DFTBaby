...
def readCube(cubefile):
    ...
    parts = l[2].split()
    natoms, origin = int(parts[0]), np.array(list(map(float, parts[1:4])))
    # voxels and axis vectors
    Nvox = [0, 0, 0]
    axes = [None, None, None]
    for xyz in range(0, 3):
        parts = l[3+xyz].split()
        Nvox[xyz], axes[xyz] = int(parts[0]), np.array(list(map(float, parts[1:])))

    atomlist = []
    for i in range(0, natoms):
        parts = l[6+i].split()
        Zi, posi = int(parts[0]), list(map(float, parts[2:]))
        atomlist.append((Zi, posi))
    # read grid into long list of floats
    data1d = []
    for dataline in l[7+i:]:
        data1d += list(map(float, dataline.split()))
    Nx,Ny,Nz = Nvox
...
def writeCube(cubefile, atomlist, origin, axes, data):
    ...
    for i in range(nx):
        for j in range(ny):
            for k in range(nz):
                print(" %11.5e" % data[i,j,k], end=' ', file=fh)
                if k % 6 == 5: print("\n ", end=' ', file=fh)
            print("\n ", end=' ', file=fh)
