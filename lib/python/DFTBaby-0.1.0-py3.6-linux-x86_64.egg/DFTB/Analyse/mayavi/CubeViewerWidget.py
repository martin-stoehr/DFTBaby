...
class CubeData:
    ...
    def loadFromFile(self, cube_file):
        ...
        try:
            ...
            #  e.g.   1 2201
            int1, int2 = list(map(int, lines[0].split()))
            # skipt extra line
            lines = lines[1:]
        except ValueError:
            pass
...
class ChargeCloudRadii:
    def __init__(self):
        ...
        for Z,U in list(hubbard_U_byZ.items()):
            self.sigmas[Z] = 1.0/(np.sqrt(np.pi)*U)
    def setIsoValue(self, iso):
        logIso = np.log(iso)
        for Z,sA in list(self.sigmas.items()):
...
class QCubeViewerWidget(QtGui.QWidget):
    def __init__(self, parent=None):
        ...
        for action in list(self.options.values()):
    ...
    def selectShowOptions(self, options=["surfaces", "charges"]):
        ...
        # hide all options
        for o in list(self.options.values()):
            o.setVisible(False)
