...
def fit_hueckel(enQ, Nmax=12):
    ...
    N = np.array(list(range(2, len(enQ)+1)))
    ...
    plt.plot(xfit, m*(xfit)+c, ls="-", lw=2, label="fit to H\\u00FCckel Model, $%3.0f N + %3.0f$ (nm)" % (m,c))
    plt.legend(loc="lower center")
    plt.show()

def fit_exciton_splitting(splitting_energies, Nmax=4):
    N = np.array(list(range(2, len(splitting_energies)+1)))
    # fit a line m*x + c through the data
    x = np.cos(np.pi/(N+1))
    # monomer is not included
