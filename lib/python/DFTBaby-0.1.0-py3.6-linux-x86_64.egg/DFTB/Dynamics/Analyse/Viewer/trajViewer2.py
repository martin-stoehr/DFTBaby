...
def read_xyz_datafields_it(filename, units="Angstrom", fragment_id=atomic_number):
    ...
    while 1:
        ...
        for i in range(nat):
            ...
            datafield.append( list(map(float,words[4:])) )
            if units == "Angstrom":
...
class Main(QtGui.QMainWindow):
    ...
    def changeTrajectory(self):
        ...
        trajs = list(map(int, [s.text() for s in selected]))
        self.geometries = []
        self.nsteps = 0
        print("Selected: %s" % list(map(str, trajs)))
    ...
    def renderMovie(self):
        ...
        if ok == False:
            return
        steps = list(range(0, self.nsteps))
    ...
    def plotTrajectory(self, trajdir, trajID):
        ...
        self.currStateAxis.set_yticks(list(range(0, len(self.trajdirs))))
