...
def read_force_field(ff_file, units="Angstrom", fragment_id=AtomicData.atomic_number):
    ...
    for i in range(nat+3):
        ...
        if units == "Angstrom":
            x,y,z = [c/AtomicData.bohr_to_angs for c in [x,y,z]]
        if words[0] == "Tv":
...
def write_force_field(ff_file, atomlist, atomtypes, partial_charges, lattice_vectors=[],
    ...
    for i in range(nat):
        ...
        if units == "Angstrom":
            x,y,z = [c*AtomicData.bohr_to_angs for c in [x,y,z]]
        print(" %4s    %15.10f %15.10f %15.10f    %3.1d  %+5.3f" % (atname.capitalize(), x,y,z, type_num, charge), file=fh)
    # lattice vectors
...
def read_transition_charges(chromo_file, units="Angstrom"):
    ...
    while True:
        ...
        line = fh.readline()
        excitation_energies = [float(f) / AtomicData.hartree_to_eV for f in line.split()]
...
def write_transition_charges(chromo_file, atomlist,
    ...
    # excitation energies
    Nst = len(excitation_energies)
    print("%s" % " ".join(["%10.5f" % (f*AtomicData.hartree_to_eV) for f in excitation_energies]), file=fh)
    #
...
def enlarged_unitcell(atomlist, lattice_vectors, nmax=1):
    ...
    for dim in range(0, 3):
        if norm2[dim] == 0.0:
            ...
            ns[dim] = list(range(-nmax,nmax+1))
    print("replica cells: %s" % list(map(len, ns)))
