...
class metadynamics:
    ...
    def read_input(self):
        ...
        for idx, cv in enumerate(self.cfg["collective variables"]):
            ...
            if "plot" in list(cv.keys()):
                self.cvs[-1].set_plot(cv["plot"])
        if "well-tempered" in list(self.cfg.keys()):
            self.welltemp = True
...
class cv_cn(cv):
    def __init__(self, idx, cfg, symb):
        cv.__init__(self, idx, cfg, symb)
        self.atom = cfg["atom"]
        if "reference" in list(cfg.keys()):  # if reference atoms are defined, get indices
            self.ref = cfg["reference"]
        else:  # if not, list of reference atoms has to be every other atom
            self.ref = list(range(self.atom))+list(range(self.atom+1, len(self.symbols)))
        self.n = cfg["n"]
        self.m = cfg["m"]
