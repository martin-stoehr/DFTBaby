...
def assign_bond_orders(atomlist, ConMat, charge=0, latte_file=""):
    ...
    for I in range(0, Nat):
        Zi = atomlist[I][0]
        if Zi in list(formal_charge_constraints.keys()):
            Nc += 1
            # hydrogen or carbon
            d = np.zeros(N)
    ...
    for I in range(0, Nat):
        Zi = atomlist[I][0]
        if Zi in list(fc_lower_limit.keys()):
            Nfc += 1
            #
            d = np.zeros(N)
    ...
    for I in range(0, Nat):
        Zi = atomlist[I][0]
        if Zi in list(fc_upper_limit.keys()):
            d = np.zeros(N)
...
def latte_input(latte_file, A_eq, b_eq, A_ub, b_ub):
    ...
    for i in range(0, m):
        print("%2d  " % b_eq[i], end=' ', file=fh)
        for j in range(0, d):
            print("%2d " % (-A_eq[i,j]), end=' ', file=fh)
        print("", file=fh)
    # upper bound constraints
    for i in range(0, n):
        print("%2d " % b_ub[i], end=' ', file=fh)
        for j in range(0, d):
            print("%2d " % (-A_ub[i,j]), end=' ', file=fh)
        print("", file=fh)
    # Which constraints should be treated as equality constraints?
    print("linearity %d " % m, end=' ', file=fh)
    for i in range(0, m):
        print("%d " % (i+1), end=' ', file=fh)
    print("", file=fh)
    print("nonnegative %d " % d, end=' ', file=fh)
    for j in range(0, d):
        print("%d " % (j+1), end=' ', file=fh)
    print("", file=fh)
    fh.close()
