...
class CrystalSystem:
    ...
    def translation_vectors(self, nmax, space="real", include_origin=True):
        ...
        klm = list(itertools.product(list(range(-nmax[0]+1, nmax[0])),list(range(-nmax[1]+1,nmax[1])), list(range(-nmax[2]+1,nmax[2]))))
        Ts = []
        for int_disp in klm:
    ...
    def nearest_neighbours_r(self, Rmax):
        ...
        ns = list(itertools.product(*[list(range(-Nmax[i], Nmax[i]+1)) for i in range(0, self.dim)]))
        print(ns)
