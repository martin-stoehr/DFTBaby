...
class ReppotFitter():
    ...
    def add_polyatomic_curve(self, geometries, forces_wo_frep, forces_with_frep, \
        ...
        for atomlist, f_wo_rep, f_with_rep in zip(geometries, forces_wo_frep, forces_with_frep):
            ...
            for (i,j) in list(bond_lengths.keys()):
...
if __name__ == "__main__":
    ...
    for line in sys.stdin.readlines():
        ...
        keywords = dict([tuple(s.replace(" ","").replace("\t","").split("=")) for s in keywords_str.split()])
        ...
        if active_atoms_str == "":
            ...
            active_atoms = list(map(int, active_atoms_str.split(",")))
