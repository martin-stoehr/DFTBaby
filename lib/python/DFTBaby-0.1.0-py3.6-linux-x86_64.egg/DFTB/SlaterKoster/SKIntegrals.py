...
from . import slako_transformations as T
from . import slako_transformations_dipole as Tdip
...
class Atom:
    ...
    def getValenceOrbitals(self):
        ...
        for i,indx in enumerate(self.atom.valence_orbitals):
            ...
            assert not l in list(R_valence.keys()) # assume that all valence orbitals have different l
...
class AtomPair:
    ...
    def getSKIntegrals(self, d, grid):
        ...
        for i in list(T.index2tau.keys()):
...
#            self.H[(l1,l2,i)] = en2*olap + Hpart
        ...
        for i in list(Tdip.index2tau.keys()):
...
#        show()
    def write_py_slako(self, slako_dir):
        ...
        print("index2symbol = \\\n%s" % dict([(i, T.tau2symbol[T.index2tau[i]]) for (l1,l2,i) in list(self.S.keys())]), file=fh)
...
def read_slakoformat_skf(skfile, sk=None, atom_order="AB"):
    ...
    if at1 == at2:
        # line 2
        # homonuclear case
        Ed, Ep, Es, SPE, Ud, Up, Us, fd, fp, fs = list(map(float, process_slako_line(fh.readline())))
    # line 2 or 3
    mass, c2, c3, c4, c5, c6, c7, c8, c9, rcut, d1, d2, d3, d4, d5, d6, d7, d8, d9, d10 = \
        list(map(float, process_slako_line(fh.readline())))
...
def combine_slako_tables_f90(SKT):
    ...
    Zs = list(set([Z1 for (Z1,Z2) in list(SKT.keys())]))
    Zs.sort()
    Ntype = len(Zs)
    ...
    for (Zi,Zj),sktab in SKT.items():
        # S and H0
        tab_indeces = list(sktab.S_tck.keys())
        ...
        if sktab.haveDipoles():
            tab_indeces = list(sktab.Dipole_tck.keys())
