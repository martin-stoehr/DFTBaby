...
#        self.density_fitter = DensityFitting(atomlist, self.hubbard_U)
    ...
    def setGeometry(self, atomlist, charge=0.0, keep_charge=False, update_symmetry=True):
        ...
        for i,(Zi,posi) in enumerate(self.atomlist):
            self.orbitals_on_atom[i] = list(range(self.dim, self.dim + len(self.valorbs[Zi])))
        ...
        if self.verbose > 0:
            ...
            print("  Geometry")
            print("  ========")
            print(str.ljust("Atom",15) + str.center("X/bohr",10) + str.center("Y/bohr",10) + str.center("Z/bohr",10))
            print("-"*45)
            for (Zi, posi) in self.atomlist:
                print(str.ljust(atom_names[Zi-1], 15) \
                    + str.rjust("%.5f" % posi[0],10) \
                    + str.rjust("%.5f" % posi[1],10) \
                    + str.rjust("%.5f" % posi[2],10))
            print("")
    def setPointCharges(self, point_charges):
        ...
        if self.verbose > 0 and len(self.point_charges) > 0:
            print("  Point Charges")
            print("  =============")
            print(str.ljust("Charge",15) + str.center("X/bohr",10) + str.center("Y/bohr",10) + str.center("Z/bohr",10))
            print("-"*45)
            for (Zi, posi) in point_charges:
                print(str.ljust("%.3f" % Zi, 10) \
                    + str.rjust("%.5f" % posi[0],10) \
                    + str.rjust("%.5f" % posi[1],10) \
                    + str.rjust("%.5f" % posi[2],10))
            print("")
...
#            q_mu = sum(Qk,axis=1)
    ...
    def write_Mulliken_charges(self):
        ...
        txt += "  =================================\n"
        for i,(Zi,posi) in enumerate(self.atomlist):
            txt += "%s: %.3f\t(%.3f)\n" % (str.center("%s-%s" % (atom_names[Zi-1], i),12), self.q[i].real, self.dq[i].real)
        print(txt)
    ...
    def runPeriodicNonSCC(self, lattice_vectors, ks, dq=None, nmax=(10,10,10)):
        ...
        T = []
        klm = list(itertools.product(list(range(-nmax[0], nmax[0]+1)), list(range(-nmax[1],nmax[1]+1)), list(range(-nmax[2],nmax[2]+1))))
...
###############################
    def loadConstraints(self, constraints_script=None):
        ...
        dic = {}
        try:
            exec(compile(open(expandvars(expanduser(constraints_script)), "rb").read(), expandvars(expanduser(constraints_script)), 'exec'), dic)
        except IOError as e:
...
###############################
    def writeIteration(self, info=["convergence", "energies", "orbitals", "charges"]):
        ...
        if "energies" in info:
            ...
            txt += "  ===================\n"
            txt += "band structure energy E_bs: %s %s\n" % (str.rjust("%.7f hartree" % self.E_bs,20), str.rjust("%.7f eV" % (self.E_bs*hartree_to_eV),20))
            txt += "Coulomb energy E_coulomb  : %s %s\n" % (str.rjust("%.7f hartree" % self.E_coulomb,20), str.rjust("%.7f eV" % (self.E_coulomb*hartree_to_eV),20))
            if hasattr(self, "E_multipoles"):
                txt += "            E_multipoles  : %s %s\n" % (str.rjust("%.7f hartree" % self.E_multipoles,20), str.rjust("%.7f eV" % (self.E_multipoles*hartree_to_eV),20))

            if hasattr(self, "Vcoul_pc"):
                txt += "point charges E_pc        : %s %s\n" % (str.rjust("%.7f hartree" % self.E_point_charges,20), \
                                                                str.rjust("%.7f eV" % (self.E_point_charges*hartree_to_eV),20))
            if hasattr(self, "E_onsite"):
                txt += "Coulomb on-site E_onsite  : %s %s\n" % (str.rjust("%.7f hartree" % self.E_onsite,20), \
                                                                str.rjust("%.7f eV" % (self.E_onsite*hartree_to_eV),20))
            if hasattr(self, "E_HF_x"):
                txt += "long range HF-x E_x       : %s %s\n" % (str.rjust("%.7f hartree" % self.E_HF_x,20), \
                                                                str.rjust("%.7f eV" % (self.E_HF_x*hartree_to_eV),20))

            txt += "total electronic energy   : %s %s\n" % (str.rjust("%.7f hartree" % self.E_elec,20), str.rjust("%.7f eV" % (self.E_elec*hartree_to_eV),20))
            txt += "repulsion energy          : %s %s\n" % (str.rjust("%.7f hartree" % self.E_rep,20), str.rjust("%.7f eV" % (self.E_rep*hartree_to_eV),20))
            if hasattr(self, "dispersion"):
                txt += "dispersion energy         : %s %s\n" % (str.rjust("%.7f hartree" % self.E_disp,20), str.rjust("%.7f eV" % (self.E_disp*hartree_to_eV),20))
            if self.qmmm != None:
                txt += "QM/MM energy              : %s %s\n" % (str.rjust("%.7f hartree" % self.E_qmmm,20), str.rjust("%.7f eV" % (self.E_qmmm*hartree_to_eV),20))
            if self.cavity != None:
                txt += "Cavity energy             : %s %s\n" % (str.rjust("%.7f hartree" % self.E_cav,20), str.rjust("%.7f eV" % (self.E_cav*hartree_to_eV),20))
            if hasattr(self, "solvent_cavity") and self.solvent_cavity.implicit_solvent != 0:
                txt += "solvent screening energy  : %s %s\n" % (str.rjust("%.7f hartree" % self.E_screening,20), str.rjust("%.7f eV" % (self.E_screening*hartree_to_eV),20))
            txt += "total energy              : %s %s\n" % (str.rjust("%.7f hartree" % self.E_tot,20), str.rjust("%.7f eV" % (self.E_tot*hartree_to_eV),20))
        if "orbitals" in info:
            txt += "HOMO-LUMO gap             : %s %s\n" % (str.rjust("%.7f hartree" % self.HLgap,20), str.rjust("%.7f eV" % (self.HLgap*hartree_to_eV),20))
            txt += "  Orbital Energies\n"
            txt += "  ================\n"
            sort_indx = argsort(self.orbe) # calculated redundantly in fermi_occupation !!
            for i,a in enumerate(sort_indx):
                txt += "%s: %s %s  (%.3f e)\n" % (str.rjust(str(i+1),6), \
                                              str.rjust("%.7f hartree" % self.orbe[a], 20), \
                                              str.rjust("%.7f eV" % (self.orbe[a]*hartree_to_eV), 20), \
                                              self.f[a])
            if self.verbose > 1:
                txt += "  MO Coefficients\n"
                txt += "  ===============\n"
        ...
        if "charges" in info:
            ...
            for i,(Zi,posi) in enumerate(self.atomlist):
                txt += "%s: %.3f\t(%+.3f)" \
                    % (str.center("%s-%s" % (atom_names[Zi-1], i),12), \
                       self.q[i], self.dq[i])
                if self.mulliken_dipoles == 1:
                    txt += "\t[%+.3f %+.3f %+.3f]\n" \
...
if __name__ == "__main__":
    ...
    for i,atomlist in enumerate(structures):
        ...
        print("TOTAL ENERGY OF STRUCTURE %d              : %s %s\n" % (i, str.rjust("%.7f hartree" % dftb2.E_tot,20), str.rjust("%.7f eV" % (dftb2.E_tot*hartree_to_eV),20)))
