...
#
    ...
    def _writeExcitations(self, print_max_states=1000000, nr_dominant_ex=2, print_MO_tdip=False):
        ...
        if self.dftb2.use_symmetry > 0:
            ...
            symstr = "   "
        txt = " ground state energy: %s %s\n\n" % (str.rjust("%.7f hartree" % self.en0,15), \
                               str.rjust("%.7f eV" % (self.en0*hartree_to_eV), 15))
        txt += "           Excited States\n"
        txt += "           ==============\n"
        ...
        for I in range(0, min(print_max_states, len(self.Omega))):
            ...
            txt += "%s %s %s: %s %s %s  %s %s %s %s %s\n" % (str.rjust(str(I+1),5), \
                               self.multiplicity, \
                               str.rjust(self.Irreps[I], 3), \
                               str.rjust("%.7f hartree" % self.Omega[I],20), \
                               str.rjust("%.7f eV" % (self.Omega[I]*hartree_to_eV), 17), \
                               str.rjust("%.7f nm" % (hartree_to_nm / self.Omega[I]), 17), \
                               str.rjust("%.7f" % self.oscillator_strength[I], 12), \
                               str.rjust("%s" % domex_str, 45), \
                               str.rjust("%s" % domKS_str, 8), \
                               str.rjust("[%+2.7f %+2.7f %+2.7f]" % (self.tdipX[I], self.tdipY[I], self.tdipZ[I]), 40), \
                               str.rjust("%.4f" % self.Lambda2[I], 7))
        print(txt)
        ...
        for irrep in unique_irreps:
            ...
            for I in range(0, min(max_states, len(self.Omega))):
                if self.Irreps[I] == irrep:
                    print("%s   %s" % (str.rjust("%.7f" % self.Omega[I],20), str.rjust("%.7f" % self.oscillator_strength[I],20)), file=fh)
            fh.close()
    ...
    def ParticleHoleCharges(self, I):
        ...
        if self.dftb2.verbose > 0:
            ...
            txt += "======================\n"
            for A,(ZA,posA) in enumerate(atomlist):
                txt += "%s: %3.7f (p) \t %3.7f (h)\n" % (str.center("%s-%s" % (atom_names[ZA-1], A),12), particle_charges[A], hole_charges[A])
            print(txt)
    ...
    def saveTransitionCharges(self, save_transition_charges=None):
        ...
        if save_transition_charges != None:
            ...
            print("# Mulliken transition charges (in unit of -e) between ground and excited states", file=fh)
